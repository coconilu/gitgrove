...
后端 `projects_v2.rs` 走 GraphQL：`list_projects_v2`（owner 为空走 viewer，ownerType 不给时同查 user/org 两路径）与 `get_project_v2`（按 project node id 读 fields + items，内存缓存 5 分钟，`refresh=true` 强制刷新）。
